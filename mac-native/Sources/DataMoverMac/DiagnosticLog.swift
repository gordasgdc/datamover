...
///   - unified log, subsistemul `dev.gordas.datamover` — `log show`, Console;
///   - `~/Library/Logs/DataMover.log` — citibil direct în terminal, o linie
...
struct DiagnosticLog {
    ...
    static let subsystem = "dev.gordas.datamover"
    static let fileURL = FileManager.default.homeDirectoryForCurrentUser
        .appendingPathComponent("Library/Logs/DataMover.log")
    /// Peste plafon, fișierul devine `DataMover.log.1` și se începe unul nou
    /// (Regula 21: nimic nu crește la nesfârșit).
    static let maxBytes: UInt64 = 5 * 1024 * 1024

    /// `defaults write dev.gordas.datamover DataMover.verboseLog -bool true`
    /// trimite și mesajele de nivel debug în fișier (unified log le are oricum).
    static var verbose: Bool { UserDefaults.standard.bool(forKey: "DataMover.verboseLog") }
...
private final class LogFile {
    ...
    private let queue = DispatchQueue(label: "dev.gordas.datamover.logfile", qos: .utility)
