...
enum AppMover {
    ...
    static let destination = URL(fileURLWithPath: "/Applications/DataMover.app")
    private static let quarantineKey = "com.apple.quarantine"
    private static let translocateFlag: UInt32 = 0x0080

    /// Apelat o singură dată, după `didFinishLaunching` (vezi `DataMoverMacApp`).
    ...
    private static func askToMove() -> Bool {
        ...
        let alert = NSAlert()
        alert.messageText = L.t("appMover.title")
        alert.informativeText = L.t("appMover.message")
        alert.addButton(withTitle: L.t("appMover.move"))
        alert.addButton(withTitle: L.t("appMover.notNow"))
    ...
    private static func move(from source: URL, original: URL) {
        do {
            ...
            guard installed == version(of: Bundle.main.bundleURL) else {
                throw failure(String(format: L.t("appMover.versionMismatch"), installed ?? "?"))
            }
            ...
            if let flags = quarantineFlags(destination), flags & translocateFlag != 0 {
                throw failure(L.t("appMover.stillTranslocated"))
            }
    ...
    private static func fail(_ error: Error) {
        ...
        let wasAccessory = bringToFront()
        let alert = NSAlert()
        alert.messageText = L.t("appMover.failedTitle")
        alert.informativeText = String(format: L.t("appMover.failedMessage"), error.localizedDescription)
    ...
    private static func runAsAdmin(_ shell: String) throws {
        ...
        guard process.terminationStatus == 0 else {
            ...
            throw failure(message.isEmpty ? L.t("appMover.accessDenied") : message)
        }
    }
}
