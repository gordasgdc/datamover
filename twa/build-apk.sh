#!/usr/bin/env bash
# =============================================================================
# DataMover — build APK (TWA) din PWA-ul de pe gordas.dev/datamover/
# -----------------------------------------------------------------------------
# Ce face, pe pasi:
...
if [ ! -d "app" ]; then
  ...
  bubblewrap init --manifest="https://gordas.dev/datamover/manifest.webmanifest" --directory=.
...
GATA. Mai ai de facut, manual:
  1. Copiaza docs/.well-known/assetlinks.json in RADACINA domeniului gordas.dev,
     ca sa fie servit la https://gordas.dev/.well-known/assetlinks.json
     (NU la /datamover/.well-known/ — verificarea se face pe origin, nu pe cale).
